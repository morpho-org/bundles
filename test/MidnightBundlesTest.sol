...
contract MidnightBundlesTest is Test {
    ...
    function testUnauthorized() public {
        ...
        midnightBundles.midnightBundlesBuyWithUnitsTargetAndWithdrawCollateral(
            100,
            0,
            lender,
            _noPermit(),
    ...
    function testSellUnitsTarget(uint256 offerUnits0, uint256 offerUnits1, uint256 units) public {
        ...
        if (offerUnits1 >= units - fromOffer0) {
            vm.prank(borrower);
            midnightBundles.midnightBundlesSupplyCollateralAndSellWithUnitsTarget(
    ...
    function testBuyBuyerAssetsTarget(uint256 offerUnits0, uint256 offerUnits1, uint256 targetBuyerAssets) public {
        ...
        if (offerUnits1 >= units - fromOffer0) {
            vm.prank(lender);
            midnightBundles.midnightBundlesBuyWithAssetsTargetAndWithdrawCollateral(
                targetBuyerAssets,
                0,
                lender,
                _noPermit(),
    ...
    function testSellSellerAssetsTarget(uint256 offerUnits0, uint256 offerUnits1, uint256 targetSellerAssets) public {
        ...
        if (offerUnits1 >= neededFromOffer1) {
            vm.prank(borrower);
            midnightBundles.midnightBundlesSupplyCollateralAndSellWithAssetsTarget(
                targetSellerAssets,
                type(uint256).max,
                borrower,
                borrower,
    ...
    function testRepayWithReferralFee(uint256 units, uint256 assets, uint256 referralFeePct) public {
        ...
        vm.prank(borrower);
        midnightBundles.midnightBundlesRepayAndWithdrawCollateral(
            market,
            assets,
            borrower,
            _noPermit(),

...
contract MidnightBundlesV2MakerTest is Test {
    ...
    function testMakeRevertsWhenConsumptionExceedsLimit(uint128 maxConsumed, uint128 consumed, bool useDelegate)
        public
    {
    ...
    function testMakeDelegatedByAuthorizedOperator(bool useRateRatifier, bool useSignature) public {
        ...
        vm.prank(operator);
        midnight.setConsumed(cancelledGroup, type(uint128).max, operator);

        Offer memory offer = makeOffer(keccak256("delegated group"), PARKED_ASSETS, MAX_TICK / 2);
        offer.start = block.timestamp;
        if (useRateRatifier) offer.ratifier = address(rateRatifier);
        bytes32 root = offerRoot(offer);
        bytes memory rootSignature = useSignature
            ? signRoot(offer.ratifier, lender, root, true, 0, block.timestamp, lenderPrivateKey)
            : bytes("");
        ...
        assertTrue(IRatifiersV1Common(offer.ratifier).isRootRatified(lender, root), "maker root ratified");
        assertFalse(IRatifiersV1Common(offer.ratifier).isRootRatified(operator, root), "operator root untouched");
        assertTrue(midnight.isAuthorized(lender, offer.ratifier), "maker authorizes ratifier");
        assertFalse(midnight.isAuthorized(operator, offer.ratifier), "operator authorization untouched");
        assertEq(midnight.consumed(lender, cancelledGroup), type(uint128).max, "maker group cancelled");
        ...
        (uint256 buyerAssets,) = take(offer, root, 100e18);
        assertGt(buyerAssets, 0, "delegated offer is takeable");
        assertEq(morpho.expectedSupplyAssets(blueMarket, callbackOf(lender)), PARKED_ASSETS - buyerAssets);
    }

    function testMakeRevertsWhenCallerIsNotAuthorizedByMaker(bool useRateRatifier, bool useSignature) public {
        address ratifier = useRateRatifier ? address(rateRatifier) : address(priceRatifier);

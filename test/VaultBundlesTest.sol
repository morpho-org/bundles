...
contract VaultBundlesTest is Test {
    ...
    /// @dev The initiator is reset when each entrypoint returns, so several guarded entrypoints can be batched in a single multicall. The deposit then withdraw round trip leaves the user's balances unchanged; with fresh vaults the share price is exactly 1, so shares equal assets.
    function testMulticallDepositWithdraw(uint256 assets) public {
        ...
        calls[1] = Call(
            abi.encodeCall(
                ...
                (address(vaultV1), 0, assets, noSharesPermit, 0, address(0), block.timestamp)
            ),
            false
        );
        bundles.multicall(calls);
    ...
    function testAlreadyInitiated() public {
        ...
        bundles.vaultBundlesV1Withdraw(address(reentrantVault), 1, 0, reenteringPermit, 0, address(0), block.timestamp);
    }
}
...
contract ReentrantVault {
    ...
    function permit(address, address, uint256, uint256, uint8, bytes32, bytes32) external {
        bundles.vaultBundlesV1Withdraw(
            address(this), 1, 0, Permit(0, 0, 0, 0, bytes32(0), bytes32(0)), 0, address(0), block.timestamp
        );
    }
}

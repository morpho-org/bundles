...
contract VaultExitBundlesV1 is IVaultExitBundlesV1, IMorphoSupplyCallback, IMorphoFlashLoanCallback {
    ...
    /// @dev Passing as marketParamsList a superset of withdrawQueue makes the call immune to vault allocation changes: the full vault allocation is taken into account.
    ...
    /// @dev The in-kind portion is withdrawn in a number of iterations that is bounded by N the number of markets in the adapter (each of them can result in a rounding error against the users). An additional withdrawal rounding unit can occur when idle assets are withdrawn. The assets value of the sum of the shares withdrawn can be greater than exitAssets, but no greater than exitAssets+N+1 (or exitAssets+N if idle assets are zero).
    /// @dev Requires Morpho Blue to have at least assetsToDeallocate in loan token balance.
    /// @dev Requires the sender to have enough shares to withdraw exitAssets+N+1.
    ...
    /// @dev It's acknowledged that it is possible to call this function with duplicate markets in the list.
    /// @dev Passing as marketParamsList a superset of the adapter's market list makes the call immune to vault allocation changes.
    ...
    function vaultExitBundlesV1InKindRedemptionVaultV2(
        ...
        uint256 assetsToWithdrawFromIdle = UtilsLib.min(IERC20(asset).balanceOf(vault), exitAssets);
        if (assetsToWithdrawFromIdle > 0) {
            IVaultV2(vault).withdraw(assetsToWithdrawFromIdle, address(this), msg.sender);
            SafeTransferLib.safeTransfer(asset, msg.sender, assetsToWithdrawFromIdle);
        }

        TokenLib.forceApproveMax(asset, BLUE);

        uint256 penalty = IVaultV2(vault).forceDeallocatePenalty(adapter);
        uint256 assetsToDeallocate = (exitAssets - assetsToWithdrawFromIdle).mulDivDown(WAD, WAD + penalty);
    ...
    /// @dev If msg.sender is a vault fee recipient, totalSharesBurnt is underestimated (fee shares accrue to it during the withdrawals), weakening the share price check.
    function vaultExitBundlesV1ForceWithdrawVaultV2(
        ...
        uint256 totalSharesBurnt = sharesBefore - IERC20(vault).balanceOf(msg.sender);
        require(
            totalSharesBurnt == 0 || withdrawn.mulDivDown(1e27, totalSharesBurnt) >= minSharePriceE27,
            SlippageExceeded()
        );

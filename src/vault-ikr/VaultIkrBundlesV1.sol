...
contract VaultIkrBundlesV1 is IVaultIkrBundlesV1, IMorphoSupplyCallback, IMorphoFlashLoanCallback {
    ...
    /// @dev The assetsToDeallocate amount is floor(remainingAssets * WAD / (WAD + penalty)), where remainingAssets is what is left of forceWithdrawAssets after the penalty-free withdrawal.
    /// @dev The remainder is force deallocated by looping over the adapter's markets, taking from each market as much as its liquidity and the adapter's position allow before moving to the next one.

...
contract MidnightBundlesV2 is IMidnightBundlesV2 {
    ...
    /// @dev Set wrapAssetsToPark, or a collateral supply's wrapNative, to fund that transfer by wrapping msg.value into its token (which must then be the wrapped-native token) instead of pulling it, and its amount must equal msg.value.
    /// @dev Since msg.value funds a single wrap, exactly one transfer may be flagged. Reverts if msg.value is not consumed, which is the case when no transfer is flagged.
    ...
    function midnightBundlesV2CancelAndMake(
        ...
        if (assetsToPark > 0) {
            ...
            TokenLib.transferFromOrWrapNative(blueMarket.loanToken, msg.sender, assetsToPark, wrapAssetsToPark);
        ...
        for (uint256 i; i < collateralSupplies.length; i++) {
            ...
            if (collateralSupply.assets > 0) {
                ...
                TokenLib.transferFromOrWrapNative(
                    collateralToken, msg.sender, collateralSupply.assets, collateralSupply.wrapNative
                );
    ...
    /// @dev With wrapBuyerAssets, maxBuyerAssets is wrapped from msg.value into market.loanToken (which must then be the wrapped-native token) instead of being pulled, so msg.value must equal maxBuyerAssets, and the remaining tokens are unwrapped back to native.
    ...
    /// @dev Set a collateral supply's wrapNative to fund it by wrapping msg.value into its token (which must then be the wrapped-native token) instead of pulling it, and its assets must equal msg.value.
    /// @dev Since msg.value funds a single wrap, exactly one supply may be flagged. Reverts if msg.value is not consumed, which is the case when no supply is flagged.
    function midnightBundlesV2SupplyCollateralAndSellWithUnitsTarget(
        ...
        for (uint256 i; i < collateralSupplies.length; i++) {
            ...
            TokenLib.transferFromOrWrapNative(
                token, msg.sender, collateralSupplies[i].assets, collateralSupplies[i].wrapNative
            );
    ...
    /// @dev The collateralReceiver will receive collateralWithdrawals[0].assets of the first token of collateralWithdrawals, etc.
    /// @dev With wrapBuyerAssets, targetBuyerAssets is wrapped from msg.value into market.loanToken (which must then be the wrapped-native token) instead of being pulled, so msg.value must equal targetBuyerAssets.

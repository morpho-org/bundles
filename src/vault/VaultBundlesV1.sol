...
contract VaultBundlesV1 is IVaultBundlesV1 {
    ...
    /// @dev When native tokens are sent, assetPermit.kind must be PermitKind.None and assets must equal msg.value; the native tokens are wrapped into the vault asset (which must be the wrapped-native token) instead of pulling.

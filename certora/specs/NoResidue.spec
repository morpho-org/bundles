...
rule withdrawPreservesBalance(env e, BlueBundlesV1.MarketParams marketParams, uint256 assets, uint256 shares, uint256 minSharePriceE27, BlueBundlesV1.Signature authorizationSignature, uint256 authorizationNonce, uint256 feePct, address recipient, address token, uint256 deadline) {
    require e.msg.sender != currentContract, "bundler is never its own caller";
    require recipient != currentContract, "no bundler donations of the fee";

    mathint before = bundlerBalance[token];
    blueBundlesV1Withdraw(e, marketParams, assets, shares, minSharePriceE27, authorizationSignature, authorizationNonce, feePct, recipient, deadline);
    assert bundlerBalance[token] == before;
}

rule supplyCollateralAndBorrowPreservesBalance(env e, BlueBundlesV1.MarketParams marketParams, uint256 collateralAmount, uint256 borrowAssets, uint256 minSharePriceE27, uint256 maxLtv, TokenLib.TokenPermit permit, BlueBundlesV1.Signature authorizationSignature, uint256 authorizationNonce, uint256 feePct, address recipient, address token, uint256 deadline) {
    require permit.kind == TokenLib.PermitKind.None, "simplification for prover performance";
    require e.msg.sender != currentContract, "bundler is never its own caller";
    require recipient != currentContract, "no bundler donations of the fee";

    mathint before = bundlerBalance[token];
    blueBundlesV1SupplyCollateralAndBorrow(e, marketParams, collateralAmount, borrowAssets, minSharePriceE27, maxLtv, permit, authorizationSignature, authorizationNonce, feePct, recipient, deadline);
    assert bundlerBalance[token] == before;
}
...
rule repayAndWithdrawCollateralPreservesBalance(env e, BlueBundlesV1.MarketParams marketParams, uint256 assets, uint256 shares, uint256 maxRepayAssets, uint256 maxSharePriceE27, uint256 withdrawCollateralAssets, uint256 maxLtv, TokenLib.TokenPermit permit, BlueBundlesV1.Signature authorizationSignature, uint256 authorizationNonce, uint256 feePct, address recipient, address token, uint256 deadline) {
    ...
    mathint before = bundlerBalance[token];
    blueBundlesV1RepayAndWithdrawCollateral(e, marketParams, assets, shares, maxRepayAssets, maxSharePriceE27, withdrawCollateralAssets, maxLtv, permit, authorizationSignature, authorizationNonce, feePct, recipient, deadline);
    assert bundlerBalance[token] == before;
}

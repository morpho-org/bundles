...
// Two assumptions shared with the BlueBundles suite:
...
//   - non-reentrant vault and token: the permit and approve calls are unresolved, so reentering an entry point is out of model.
...
rule withdrawPreservesBalance(env e, address vault, uint256 assets, uint256 shares, uint256 minSharePriceE27, TokenLib.Permit sharesPermit, uint256 feePct, address recipient, address token, uint256 deadline) {
    require e.msg.sender != currentContract, "bundler is never its own caller";
    require recipient != currentContract, "no bundler donations of the fee";

    mathint before = bundlerBalance[token];
    vaultBundlesV1Withdraw(e, vault, assets, shares, minSharePriceE27, sharesPermit, feePct, recipient, deadline);
    assert bundlerBalance[token] == before;
}

rule migratePreservesBalance(env e, address sourceVault, address destVault, uint256 assetsWithdrawn, uint256 sharesRedeemed, uint256 sourceMinSharePriceE27, uint256 destMaxSharePriceE27, TokenLib.Permit sharesPermit, uint256 feePct, address recipient, address token, uint256 deadline) {
    require e.msg.sender != currentContract, "bundler is never its own caller";
    require recipient != currentContract, "no bundler donations of the fee";

    mathint before = bundlerBalance[token];
    vaultBundlesV1Migrate(e, sourceVault, destVault, assetsWithdrawn, sharesRedeemed, sourceMinSharePriceE27, destMaxSharePriceE27, sharesPermit, feePct, recipient, deadline);
    assert bundlerBalance[token] == before;
}
